...
GUI::GUI() noexcept
{
    ...
    Fonts::pixel = io.Fonts->AddFontFromMemoryTTF(smallest_pixel, sizeof(smallest_pixel), 10.f, 0, Helpers::getFontGlyphRanges());
    Fonts::indicators = io.Fonts->AddFontFromMemoryTTF(gabarito, sizeof(gabarito), 28.f, 0, Helpers::getFontGlyphRanges());
    // Load Fonts
    ...
    Fonts::weaponIcons = io.Fonts->AddFontFromMemoryTTF((void*)weaponIcon, sizeof(weaponIcon), 12.0f, NULL, io.Fonts->GetGlyphRangesCyrillic());
...
void GUI::renderRagebotWindow() noexcept
{
    ...
    static bool hitbox[ARRAYSIZE(hitboxes)] = { false, false, false, false, false, false, false };
    static std::string previewvalue = "";
    bool once = false;
    static int currentCategory{ 0 };
    ImGui::PushItemWidth(110.0f);
    ImGui::PushID(0);
    ImGui::Combo(skCrypt("Weapon"), &currentCategory, skCrypt("Global\0Pistols\0Deagle\0R8 Revolver\0SSG-08\0AWP\0Auto snipers\0Zeus x27\0Others\0"));
    ImGui::SameLine();
    ImGui::Checkbox(c_xor("Override config"), &config->rageBot[currentCategory].override);
    ImGui::Columns(2, nullptr, false);
    ImGui::SetColumnOffset(1, 220.0f);
    ImGui::Checkbox(c_xor("Enable ragebot"), &config->ragebot.enabled);
    ImGui::Checkbox(c_xor("Silent viewangles"), &config->ragebot.silent);
    ImGui::Checkbox(c_xor("Knifebot"), &config->ragebot.knifebot);
    ImGui::Checkbox(c_xor("Auto shot"), &config->ragebot.autoShot);
    ImGui::Checkbox(c_xor("Auto scope"), &config->ragebot.autoScope);
    ImGui::Checkbox(c_xor("Auto stop"), &config->rageBot[currentCategory].autoStop);
    ...
    {
        multi[i] = (config->rageBot[currentCategory].autoStopMod & 1 << i) == 1 << i;
    }
    ...
    for (size_t i = 0; i < ARRAYSIZE(multicombo_items); i++)
    {
        ...
        if (multi[i])
        {
            ...
            config->rageBot[currentCategory].autoStopMod |= 1 << i;
        }
        else
        {
            config->rageBot[currentCategory].autoStopMod &= ~(1 << i);
    ...
    {
        hitbox[i] = (config->rageBot[currentCategory].hitboxes & 1 << i) == 1 << i;
    }
    ...
    for (size_t i = 0; i < ARRAYSIZE(hitboxes); i++)
    {
        ...
        if (hitbox[i])
        {
            ...
            config->rageBot[currentCategory].hitboxes |= 1 << i;
        }
        else
        {
            config->rageBot[currentCategory].hitboxes &= ~(1 << i);
    ...
    ImGui::SliderInt(c_xor("Min damage"), &config->rageBot[currentCategory].minDamage, 0, 105, "%d");
    ImGui::PushID("Min damage override Key");
    ImGui::hotkey2("Min damage override Key", config->binds.dmgOverride);
    ImGui::PopID();
    ImGui::SliderInt("Min damage override", &config->rageBot[currentCategory].minDamageOverride, 0, 105, "%d");

    ImGui::PushID("Doubletap");
    ImGui::hotkey2("Doubletap", config->binds.doubletap);
    ImGui::PopID();
    ImGui::PushID("Hideshots");
    ImGui::hotkey2("Hideshots", config->binds.hideshots);
    ImGui::PopID();
...
void GUI::renderRageAntiAimWindow() noexcept
{
    ...
    static int current_antiaim{};
    ImGui::Combo("Conditions", &current_antiaim, "Global\0Standing\0Moving\0Chrouching\0Slow-walking\0Jumping\0Chrouch jumping\0");
    if (current_antiaim > 0)
        ImGui::Checkbox(("Override settings"), current_antiaim == 1 ? &config->condAA.standing : current_antiaim == 2 ? &config->condAA.moving : current_antiaim == 3 ? &config->condAA.chrouching : current_antiaim == 4 ? &config->condAA.slowwalk : current_antiaim == 5 ? &config->condAA.jumping : &config->condAA.cjumping);
    ...
    if (config->rageAntiAim[current_antiaim].yawModifier == 1 || config->rageAntiAim[current_antiaim].yawModifier == 2 || config->rageAntiAim[current_antiaim].yawModifier == 3)
        ImGui::SliderInt("Jitter yaw range", &config->rageAntiAim[current_antiaim].jitterRange, -90, 90, "%d");

    if (config->rageAntiAim[current_antiaim].yawBase == Yaw::spin)
    {
        ImGui::PushItemWidth(220.0f);
        ImGui::SliderInt("Spin base", &config->rageAntiAim[current_antiaim].spinBase, -180, 180, "%d");
        ImGui::PopItemWidth();
    }

    ImGui::Checkbox("At targets", &config->rageAntiAim[current_antiaim].atTargets);
    ImGui::Checkbox("Freestand", &config->rageAntiAim[current_antiaim].freestand);
    ImGui::SameLine();
    ImGui::PushID(124);
    ImGui::hotkey2("", config->binds.freestand, 60.f);
    ImGui::PopID();
    ImGui::hotkey2("Forward", config->binds.manualForward, 60.f);
    ImGui::hotkey2("Backward", config->binds.manualBackward, 60.f);
    ImGui::hotkey2("Right", config->binds.manualRight, 60.f);
    ImGui::hotkey2("Left", config->binds.manualLeft, 60.f);

    ImGui::NextColumn();
    ImGui::hotkey2("Invert Key", config->binds.desyncInvert, 80.0f);
    ImGui::Checkbox("Enabled", &config->rageAntiAim[current_antiaim].desync.enabled);

    ImGui::PushItemWidth(220.0f);
    ImGui::SliderInt("Left limit", &config->rageAntiAim[current_antiaim].desync.leftLimit, 0, 60, "%d");
    ImGui::PopItemWidth();

    ImGui::PushItemWidth(220.0f);
    ImGui::SliderInt("Right limit", &config->rageAntiAim[current_antiaim].desync.rightLimit, 0, 60, "%d");
    ImGui::PopItemWidth();

    ImGui::Combo("Mode", &config->rageAntiAim[current_antiaim].desync.peekMode, "Off\0Peek real\0Peek fake\0Jitter\0");
    ImGui::Combo("Lby mode", &config->rageAntiAim[current_antiaim].desync.lbyMode, "Normal\0Opposite\0Sway\0");
    ImGui::Columns(1);
}
...
void GUI::renderChamsWindow() noexcept
{
    ...
    ImGui::Combo("Material", &chams.material, "Normal\0Flat\0Animated\0Platinum\0Glass\0Chrome\0Crystal\0Silver\0Gold\0Plastic\0Glow\0Pearlescent\0Metallic\0Smoke Light\0Smoke Dark\0Water\0Bush\0MP3 Detail\0Snowflakes\0Elixir\0Embers\0Wireclouds\0Seafoam\0Skyline\0Holowarp\0Cobweb\0Ghost\0Night horizon\0Tinfoil\0");
...
void GUI::renderStreamProofESPWindow() noexcept
{
    ...
    constexpr auto getConfigShared = [](std::size_t category, const char* item) noexcept -> Shared& {
        ...
        case 3: return config->streamProofESP.weapons[item];
        case 2: return config->streamProofESP.projectiles[item];
    ...
    if (ImGui::BeginListBox("##list", { 170.0f, 300.0f })) {
        constexpr std::array categories{ "Enemies", "Allies", "Projectiles", "Weapons", "Loot Crates", "Other Entities" };
        ...
        for (std::size_t i = 0; i < categories.size(); ++i) {
            ...
            if (ImGui::BeginDragDropSource()) {
                ...
                case 3: ImGui::SetDragDropPayload("Weapon", &config->streamProofESP.weapons["All"], sizeof(Weapon), ImGuiCond_Once); break;
                case 2: ImGui::SetDragDropPayload("Projectile", &config->streamProofESP.projectiles["All"], sizeof(Projectile), ImGuiCond_Once); break;
            ...
            if (ImGui::BeginDragDropTarget()) {
                if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("Player")) {
                    ...
                    case 3: config->streamProofESP.weapons["All"] = data; break;
                    case 2: config->streamProofESP.projectiles["All"] = data; break;
            ...
            const auto items = [](std::size_t category) noexcept -> std::vector<const char*> {
                ...
                case 3: return { "Pistols", "SMGs", "Rifles", "Sniper Rifles", "Shotguns", "Machineguns", "Grenades", "Melee", "Other" };
                case 2: return { "Flashbang", "HE Grenade", "Breach Charge", "Bump Mine", "Decoy Grenade", "Molotov", "TA Grenade", "Smoke Grenade", "Snowball" };
                ...
                for (std::size_t j = 0; j < items.size(); ++j) {
                    ...
                    if (!categoryEnabled || getConfigShared(i, items[j]).enabled) {
                        ...
                        if (ImGui::BeginDragDropSource()) {
                            ...
                            case 3: ImGui::SetDragDropPayload("Weapon", &config->streamProofESP.weapons[items[j]], sizeof(Weapon), ImGuiCond_Once); break;
                            case 2: ImGui::SetDragDropPayload("Projectile", &config->streamProofESP.projectiles[items[j]], sizeof(Projectile), ImGuiCond_Once); break;
                        ...
                        if (ImGui::BeginDragDropTarget()) {
                            if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("Player")) {
                                ...
                                case 3: config->streamProofESP.weapons[items[j]] = data; break;
                                case 2: config->streamProofESP.projectiles[items[j]] = data; break;
    ...
    ImGui::SameLine();
    if (ImGui::BeginChild("##child", { 500.0f, 0.0f })) {
        auto& sharedConfig = getConfigShared(currentCategory, currentItem);

        ImGui::Checkbox(skCrypt("Enabled"), &sharedConfig.enabled);
        constexpr auto spacing = 250.0f;
        ImGuiCustom::colorPicker("Snapline", sharedConfig.snapline);
        ImGui::SameLine();
        ImGui::SetNextItemWidth(90.0f);
        ImGui::Combo(skCrypt("##1"), &sharedConfig.snapline.type, skCrypt("Bottom\0Top\0Crosshair\0"));
        ...
        if (ImGui::BeginPopup("")) {
            ...
            ImGui::Combo(skCrypt("Type"), &sharedConfig.box.type, skCrypt("2D\0" "2D corners\0" "3D\0" "3D corners\0"));
            ImGui::PopItemWidth();
            ImGui::SetNextItemWidth(275.0f);
            ImGui::SliderFloat3(skCrypt("Scale"), sharedConfig.box.scale.data(), 0.0f, 0.50f, "%.2f");
            ImGui::PopItemWidth();
            ImGuiCustom::colorPicker(skCrypt("Fill"), sharedConfig.box.fill);
            ImGui::EndPopup();
        }

        ImGui::PopID();

        ImGuiCustom::colorPicker(skCrypt("Name"), sharedConfig.name);
        ImGui::SameLine(spacing);

        if (currentCategory < 2) {
            auto& playerConfig = getConfigPlayer(currentCategory, currentItem);

            ImGuiCustom::colorPicker(skCrypt("Weapon"), playerConfig.weapon);
            ImGuiCustom::colorPicker(skCrypt("Weapon icon"), playerConfig.weaponIcon);
            ImGui::SameLine(spacing);
            ImGuiCustom::colorPicker(skCrypt("Flash Duration"), playerConfig.flashDuration);
            ...
            if (ImGui::BeginPopup("")) {
                ImGui::SetNextItemWidth(95.0f);
                ImGui::Combo(skCrypt("Type"), &playerConfig.headBox.type, skCrypt("2D\0" "2D corners\0" "3D\0" "3D corners\0"));
                ImGui::SetNextItemWidth(275.0f);
                ImGui::SliderFloat3(skCrypt("Scale"), playerConfig.headBox.scale.data(), 0.0f, 0.50f, "%.2f");
                ImGuiCustom::colorPicker("Fill", playerConfig.headBox.fill);
                ImGui::EndPopup();
            }

            ImGui::PopID();
            ImGui::Checkbox(skCrypt("Audible Only"), &playerConfig.audibleOnly);
            ImGui::SameLine(spacing);
            ImGui::Checkbox(skCrypt("Spotted Only"), &playerConfig.spottedOnly);

            ImGui::Checkbox(skCrypt("Health Bar"), &playerConfig.healthBar.enabled);
            ImGui::SameLine();
            ...
            if (ImGui::BeginPopup("")) {
                ...
                ImGui::Combo("Type", &playerConfig.healthBar.type, skCrypt("Gradient\0Solid\0Health-based\0"));
                ...
                if (playerConfig.healthBar.type == HealthBar::Gradient) {
                    ImGui::SameLine();
                    ImGuiCustom::colorPicker(skCrypt("Top"), static_cast<Color4&>(playerConfig.healthBar.TopGradient));
                    ImGui::SameLine();
                    ImGuiCustom::colorPicker(skCrypt("Bottom"), static_cast<Color4&>(playerConfig.healthBar.BottomGradient));
                }
                ImGui::Combo("HP side", &playerConfig.hpside, skCrypt("Left\0Right\0"));
                ImGui::EndPopup();
            }
            ImGui::SameLine(spacing);
            ImGuiCustom::colorPicker(skCrypt("Ammo bar"), playerConfig.ammobar);
            ImGuiCustom::colorPicker(skCrypt("Line of sight"), playerConfig.lineOfSight);
            ImGui::SameLine(spacing);
            ImGuiCustom::colorPicker(skCrypt("Flags"), playerConfig.flags);
            ImGui::SameLine();
            ...
            if (ImGui::BeginPopup("")) {
                ImGui::Checkbox(skCrypt("Show armor"), &playerConfig.showArmor);
                ImGui::Checkbox(skCrypt("Show bomb carrier"), &playerConfig.showBomb);
                ImGui::Checkbox(skCrypt("Show kit carrier"), &playerConfig.showKit);
                ImGui::Checkbox(skCrypt("Show money"), &playerConfig.showMoney);
                ImGui::Checkbox(skCrypt("Show fake duck"), &playerConfig.showDuck);
                ImGui::EndPopup();
            }
            ImGui::PopID();
            if (currentCategory == 0)
            {
                ImGuiCustom::colorPicker(skCrypt("Offscreen Enemies"), config->misc.offscreenEnemies, &config->misc.offscreenEnemies.enabled);
                ...
                if (ImGui::BeginPopup("")) {
                    ImGui::SliderInt(skCrypt("Triangle size"), &config->misc.offscreenEnemies.size, 0, 100);
                    ImGui::SliderInt(skCrypt("Triangle offset"), &config->misc.offscreenEnemies.offset, 0, 750);
                    ImGui::EndPopup();
                }
                ImGui::PopID();
            }
            ...
            if (currentCategory == 0) {
                ImGui::SameLine(spacing);
                ImGuiCustom::colorPicker(skCrypt("Footstep beams"), config->visuals.footsteps.footstepBeams, &config->visuals.footsteps.footstepBeams.enabled);
                ...
                if (ImGui::BeginPopup("")) {
                    ImGui::SliderInt(skCrypt("Footstep beam radius"), &config->visuals.footsteps.footstepBeamRadius, 5, 400, "%d");
                    ImGui::SliderInt(skCrypt("Footstep beam thickness"), &config->visuals.footsteps.footstepBeamThickness, 1, 5, "%d");
                    ImGui::EndPopup();
                }
                ImGui::PopID();
            }
            ...
            ImGui::Checkbox(skCrypt("Trails"), &trails.enabled);
            ImGui::SameLine();
            ImGui::PushID("Trails");
...
void GUI::renderVisualsWindow() noexcept
{
    ...
    ImGui::SliderInt(c_xor("Zoom FOV"), &config->visuals.scope.zoomFov, 0, 100, "%d%%");
    ImGui::Checkbox(c_xor("Skip second zoom"), &config->visuals.scope.skipSecondZoom);
    ...
    ImGui::NextColumn();
    ImGui::Combo("Model scale", &config->visuals.modelscalecombo, "Off\0Smexy mode\0Fat mode\0Stickman mode\0Custom\0");
    if (config->visuals.modelscalecombo == 4)
    {
        ImGui::SliderFloat("", &config->visuals.cmslider, 0.0f, 2.0f, "model scale: %.2f");
    }
    ImGui::Checkbox("Thirdperson", &config->visuals.thirdperson.enable);
    ImGui::SameLine();
    ImGui::PushID("Thirdperson Key");
    ImGui::hotkey2("", config->binds.thirdperson);
    ImGui::PopID();
    ImGui::PushItemWidth(290.0f);
    ImGui::PushID(0);
    ImGui::SliderInt("", &config->visuals.thirdperson.distance, 0, 250, "Thirdperson distance: %d");
    ImGui::PopID();
    ...
    ImGui::Checkbox("Hit effect", &config->visuals.hitEffect);
    ...
    ImGuiCustom::colorPicker("Bullet Tracers", config->visuals.bulletTracers.color.data(), &config->visuals.bulletTracers.color[3], &config->visuals.bulletTracers.enabled);
    ImGuiCustom::colorPicker("Bullet Impacts", config->visuals.bulletImpacts.color.data(), &config->visuals.bulletImpacts.color[3], &config->visuals.bulletImpacts.enabled);
    ImGui::SliderFloat("Bullet Impacts time", &config->visuals.bulletImpactsTime, 0.1f, 5.0f, "Bullet Impacts time: %.2fs");
    ImGuiCustom::colorPicker("On Hit Hitbox", config->visuals.onHitHitbox.color.color.data(), &config->visuals.onHitHitbox.color.color[3], &config->visuals.onHitHitbox.color.enabled);
    ...
    {
        ImGuiCustom::colorPicker("Self", config->visuals.molotovPolygon.self.color.data(), &config->visuals.molotovPolygon.self.color[3]);
        ImGuiCustom::colorPicker("Team", config->visuals.molotovPolygon.team.color.data(), &config->visuals.molotovPolygon.team.color[3]);
        ImGuiCustom::colorPicker("Enemy", config->visuals.molotovPolygon.enemy.color.data(), &config->visuals.molotovPolygon.enemy.color[3]);
        ImGui::EndPopup();
    }
    ...
    {
        ImGui::PushItemWidth(290.0f);
        ImGui::Checkbox(c_xor("While scoped"), &config->visuals.viewmodelInScope);
...
void GUI::renderMiscWindow() noexcept
{
    ...
    ImGui::SliderInt("Auto strafer smoothness", &config->misc.autoStraferSmoothness, 0, 100, "%d%%");
    ImGui::Checkbox("Bunny hop", &config->misc.bunnyHop);
    ImGui::Checkbox("Fast duck", &config->misc.fastDuck);
    ImGui::Checkbox("Moonwalk", &config->misc.moonwalk);
    ImGui::Checkbox("Indicators", &config->misc.indicators.enable);
    ImGui::SameLine();
    ImGui::PushID(1385);
    if (ImGui::Button("..."))
        ImGui::OpenPopup(" ");
    ...
    ImGui::hotkey2("", config->binds.edgeJump);
    ImGui::PopID();

    ImGui::Checkbox("Slowwalk", &config->misc.slowwalk);
    ImGui::SameLine();
    ImGui::PushID("Slowwalk Key");
    ImGui::hotkey2("", config->binds.slowwalk);
    ImGui::PopID();
    ...
    ImGui::hotkey2("", config->binds.fakeduck);
    ImGui::PopID();
    ImGuiCustom::colorPicker("Auto peek", config->misc.autoPeek.color.data(), &config->misc.autoPeek.color[3], &config->misc.autoPeek.enabled);
    ImGui::SameLine();
    ImGui::PushID("Auto peek Key");
    ImGui::hotkey2("", config->binds.autoPeek);
    ImGui::PopID();
    ...
    if (ImGui::BeginPopup("")) {
        ImGui::SliderInt(c_xor("Offset"), &config->misc.offscreenEnemies.offset, 0, 750);
        ImGui::SliderInt(c_xor("Size"), &config->misc.offscreenEnemies.size, 0, 60);
        ImGui::EndPopup();
    }
    ImGui::PopID();
    ImGui::Checkbox(skCrypt("Disable model occlusion"), &config->misc.disableModelOcclusion);
    ImGui::SliderFloat(skCrypt("Aspect Ratio"), &config->misc.aspectratio, 0.0f, 5.0f, "%.2f");
    ImGui::NextColumn();
    ImGui::Checkbox(skCrypt("Disable HUD blur"), &config->misc.disablePanoramablur);        
