...
void Logger::getEvent(GameEvent* event) noexcept
{
    ...
    case fnv::hash("player_hurt"): {
        ...
        if (hurt != localPlayer->index() && attack == localPlayer->index())
        {
            ...
            log.text = c_xor("hit ") + player->getPlayerName() + c_xor(" in ") + hitgroup + c_xor(" for ") + damage + " (" + nothing /* + "\n"*/;
        }
        else if (hurt == localPlayer->index() && attack != localPlayer->index())
        {
            ...
            log.text = c_xor("hurt by ") + player->getPlayerName() + c_xor(" for ") + damage + c_xor(" in ") + hitgroup /* + "\n"*/;
        }
        break;
    }
    case fnv::hash("bomb_planted"): {
        ...
        log.text = c_xor("bomb planted on ") + site + " by " + player->getPlayerName() + c_xor(", detonation in ") + std::to_string(c4Timer->getInt()) + c_xor(" seconds") /* + "\n"*/;
        break;
    }
    ...
    case fnv::hash("bomb_begindefuse"): {
        ...
        log.text = player->getPlayerName() + c_xor(" is defusing the bomb");
        break;
    }
    default:
        return;
...
void Logger::render() noexcept
{
    ...
    for (int i = renderLogs.size() - 1; i >= 0; i--)
    {
        ...
        interfaces->surface->setTextFont(hooks->loggerFont);
