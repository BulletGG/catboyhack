...
bool reload_fonts() //offset 0x92BE3
{
...
static void __stdcall paintTraverse(unsigned int panel, bool forceRepaint, bool allowForce) noexcept
{
    ...
    if (reload_fonts())
    {
        hooks->loggerFont = interfaces->surface->createFont(); interfaces->surface->setFontGlyphSet(hooks->loggerFont, c_xor("Lucida Console"), 12, FW_NORMAL, 0, 0, 0x080 | 0x010, 0, 0);
        hooks->indicators = interfaces->surface->createFont(); interfaces->surface->setFontGlyphSet(hooks->indicators, c_xor("Calibri"), 29, 600, 0, 0, fontflag_antialias | fontflag_outline, 0, 0);
    }
...
static HRESULT __stdcall present(IDirect3DDevice9* device, const RECT* src, const RECT* dest, HWND windowOverride, const RGNDATA* dirtyRegion) noexcept
{
    ...
    if (const auto& displaySize = ImGui::GetIO().DisplaySize; displaySize.x > 0.0f && displaySize.y > 0.0f) {
        ...
        //Visuals::hitEffect(ImGui::GetBackgroundDrawList());
        Misc::drawPlayerList();
        gui->handleToggle();
...
static bool __stdcall createMove(float inputSampleTime, UserCmd* cmd, bool& sendPacket) noexcept
{
    ...
    if (localPlayer)
    {
        switch (config->visuals.modelscalecombo)
        {
        case 0:
            localPlayer->getModelScale() = config->visuals.modelscalecombo == 0 ? 1.f : 1.f; //Smexy mode
            break;

        case 1:
            localPlayer->getModelScale() = config->visuals.modelscalecombo == 1 ? 0.55f : 1.f; //Smexy mode
            break;
        case 2:
            localPlayer->getModelScale() = config->visuals.modelscalecombo == 2 ? 1.2f : 1.f; //Fat mode
            break;
        case 3:
            localPlayer->getModelScale() = config->visuals.modelscalecombo == 3 ? 0.2f : 1.f; //Stickman mode
            break;
        case 4:
            localPlayer->getModelScale() = config->visuals.modelscalecombo == 4 ? config->visuals.cmslider : 1.f; //Custom
            break;
        default:
            break;
...
static void __stdcall overrideView(ViewSetup* setup) noexcept
{
    ...
    float fover = 90 + config->visuals.fov;
    if (localPlayer)
    {
        //clip players
        bool thirdperson = config->visuals.thirdperson.enable && config->binds.thirdperson.isActive() && localPlayer->isAlive();
        if (memory->input->isCameraInThirdPerson == true && thirdperson)
        {
            int distance = config->visuals.thirdperson.distance;
        ...
        {
            if (activeWeapon->zoomLevel() > 1 && config->visuals.scope.skipSecondZoom)
            {
                activeWeapon->zoomLevel() = 1;
            }
            zoomSensitivity->setValue(0.f);
            auto zoomFov = activeWeapon->getZoomFov(activeWeapon->zoomLevel()) + config->visuals.fov;
            auto fovDelta = fover - zoomFov;
            auto totalFov = fovDelta * (1.f - config->visuals.scope.zoomFov * 0.01f);
            fover = (zoomFov + totalFov) / activeWeapon->zoomLevel();
        }
        else
        {
    ...
    if (localPlayer && localPlayer->isAlive() && config->misc.fakeduck && config->binds.fakeduck.isActive() && localPlayer->flags() & 1)
