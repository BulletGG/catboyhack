...
void Tickbase::start(UserCmd* cmd) noexcept
{
    ...
    if (!config->binds.doubletap.isActive() && !config->binds.hideshots.isActive())
    {
    ...
    if (activeWeapon->itemDefinitionIndex2() == WeaponId::Ssg08 || activeWeapon->itemDefinitionIndex2() == WeaponId::Scar20 || activeWeapon->itemDefinitionIndex2() == WeaponId::G3SG1)
        targetTickShift = 14;
    else if (config->binds.doubletap.isActive())
        targetTickShift = 12; //12 is more stable for other weapons
    else if (config->binds.hideshots.isActive())
        targetTickShift = 9;
...
bool Tickbase::canRun() noexcept
{
    ...
    if ((ticksAllowedForProcessing < targetTickShift || chokedPackets > maxUserCmdProcessTicks - targetTickShift) && memory->globalVars->realtime - realTime > config->tickbase.rechargeDelay)
    {
...
{
    config->binds.doubletap.handleToggle();
    config->binds.hideshots.handleToggle();
}
