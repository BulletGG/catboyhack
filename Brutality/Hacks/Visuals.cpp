...
void Visuals::shadowChanger() noexcept
{
    ...
    if (!config->visuals.shadowsChanger.enabled)
    {
...
void Visuals::thirdperson() noexcept
{
    ...
    const bool thirdPerson = config->visuals.thirdperson.enable && config->binds.thirdperson.isActive();
...
void Visuals::hitEffect(ImDrawList* drawList, GameEvent* event) noexcept
{
    static float lastHitTime = 0.0f;
    if (!localPlayer)
        return;

    if (event->getInt("attacker") != localPlayer->getUserId() || event->getInt("userid") == localPlayer->getUserId())
        return;
...
void Visuals::bulletTracer(GameEvent& event) noexcept
{
    ...
    case fnv::hash("bullet_impact"):
    {
        ...
        */
        ...
        beamInfo.width = 4.5f;
        beamInfo.endWidth = 4.5f;
        beamInfo.flags = FBEAM_NOTILE | FBEAM_ONLYNOISEONCE;
        ...
        if (const auto beam = memory->viewRenderBeams->createBeamPoints(beamInfo)) {
            ...
            beam->flags |= FBEAM_HALOBEAM;
            beam->die = memory->globalVars->currenttime + 2.5f;
        }
...
{
    config->binds.thirdperson.handleToggle();
}
...
void Visuals::reset(int resetType) noexcept
{
    ...
    if (resetType == 1)
    {
        ...
        const bool thirdPerson = config->visuals.thirdperson.enable && config->binds.thirdperson.isActive() && localPlayer && localPlayer->isAlive();
        if (thirdPerson)
            memory->input->isCameraInThirdPerson = false;
    }
}
